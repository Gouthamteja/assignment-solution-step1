...
public class DataMunger {
	...
	public static void main(String[] args) {
		...
		DataMunger inst= new DataMunger();
		inst.parseQuery(querystr);
		// read the query from the user into querystr variable

		// call the parseQuery method and pass the querystr variable as a parameter
	...
	public void parseQuery(String querystr) {
		...
		// call the methods
		getSplitStrings(querystr);
		getFile(querystr);
		getBaseQuery(querystr);
		getConditionsPartQuery(querystr);
		getConditions(querystr);
		getLogicalOperators(querystr);
		//getFields(querystr);
		getOrderByFields(querystr);
		getGroupByFields(querystr);
		//getAggregateFunctions(querystr);
	}
	 ...
	 */
	public String[] getSplitStrings(String querystr) {
		
		String[] splitString=querystr.toLowerCase().split(" ");
	 ...
	 */
	public String getFile(String querystr) {
		String fileName=(querystr.split("from")[1].split(" ")[1]);
	 ...
	 *    and order by clause
	 */
	public String getBaseQuery(String querystr) {
		String baseQuery= querystr.split("where")[0];
	 ...
	 */
	public String getConditionsPartQuery(String querystr) {
		String conditionPart=querystr.split("where")[1];
	 ...
	 * 2. The query might not contain where clause at all.
	 */
	public String[] getConditions(String querystr) {
		int i;
		String[] splitString=querystr.split("where")[1].split("and");
	 ...
	 * 
	 */
	public String[] getLogicalOperators(String querystr) {
		
		int i;
		String[] splitString=querystr.split("where")[1].trim().split("and");
		int n=splitString.length;
		for(i=0;i<n;i++) {
		    System.out.println("Condition"+(i+1)+":");
	 ...
	 * 
	 */
	public String[] getFields(String querystr) {
		querystr = querystr.split("from")[0];

		querystr = querystr.substring(querystr.indexOf(" "), querystr.length()).trim();

		String[] arr = querystr.split("\\s*,\\s*");;
	 ...
	 */
	public String getOrderByFields(String querystr) {
		String orderBy1=querystr.split("order by")[1].trim();
	 ...
	 */
	public String getGroupByFields(String querystr) {
		String groupBy= querystr.split("group by")[1].trim().split(" ")[0];
	 ...
	 */
	public String[] getAggregateFunctions(String querystr) {
...
//
//select * from ipl.csv where season > 2016 and city= 'Bangalore' order by win_by
//_runs